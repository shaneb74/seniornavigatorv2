...
# -------------------------------------------------------------------
# Theme + global styles (lightweight, keeps your existing variables)
# -------------------------------------------------------------------
def apply_cost_planner_theme(
    ...
    st.markdown(
        ...
        <style>
          ...
          .sn-btn.primary {{ background: var(--brand); color: white; border: none; }}
...
def drawer(title: str = "", **style):
    ...
    if title: st.subheader(title, divider=True)
    try: yield
    finally: st.markdown('</div>', unsafe_allow_html=True)

@contextlib.contextmanager
def metric_panel(gap: str = "0.5rem", **style):
    st.markdown(f'<div class="sn-metrics sn-card" style="display:grid;gap:{gap}">', unsafe_allow_html=True)
    try: yield
    finally: st.markdown('</div>', unsafe_allow_html=True)
...
def render_app_header(
    ...
    with left_col:
        ...
        if kicker: st.markdown(f"<div class='sn-kicker'>{kicker}</div>", unsafe_allow_html=True)
...
def render_module_cards(
    ...
    *, button_label: str = "Open", cols: Optional[Tuple[int, ...]] = None
) -> Dict[str, bool]:
    """
...
class NavButton:
    ...
    icon: Optional[str] = None
    # Accept legacy kw: "type" (maps to kind)
    type: Optional[str] = None  # noqa: A003 - compat alias

    def __post_init__(self):
        if self.type:
            self.kind = self.type  # compat: allow type="primary"
            self.type = None
...
def _to_navbtn(x: Union['NavButton', Dict[str, Any], None]) -> Optional['NavButton']:
    ...
    if isinstance(x, dict):
        # Support dicts like {"label": "...", "key": "...", "type": "primary", "icon": "👉"}
        return NavButton(**x)  # type maps in __post_init__
    # Fallback: treat as label only
...
def render_nav_buttons(
    ...
    *, align: str = "right", gap: str = "0.6rem"
) -> Dict[str, bool]:
    """
    ...
    with right:
        if next:
            st.markdown(f"<div style='display:flex;justify-content:flex-end;gap:{gap}'>", unsafe_allow_html=True)
...
def __getattr__(name: str):
    lname = name.lower()
    # treat container-sounding names as context managers
    if any(k in lname for k in ("panel","section","container","group","block","layout","drawer","toolbar")):
        @contextlib.contextmanager
        def _cm(*args, **kwargs):
            yield
        return _cm
    # otherwise return a tiny no-op function
    def _fn(*args, **kwargs):
        pass
    return _fn
...
def render_drawer(*, step_key: str, title: str, badge=None, description="", body=None, footer_note=None):
    ...
    with st.container(border=True):
        ...
        with left:
            ...
            if badge: st.markdown(f"<div class='pfma-note'>{badge}</div>", unsafe_allow_html=True)
        with right: pass
        st.markdown('---')
        ...
        with r:
            ...
            with c1: save = st.button("Save", key=f"{step_key}_save")
            with c2: next_clicked = st.button("Save & Continue", key=f"{step_key}_save_next")
