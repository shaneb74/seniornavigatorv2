...
cols = st.columns(2)
with cols[0]:
    st.page_link("app_pages/SeniorNav_terms.py", label="Terms of Use", icon="📄")
with cols[1]:
    st.page_link("app_pages/SeniorNav_privacy.py", label="Privacy Policy", icon="🔒")
...
if hasattr(st, "page_link"):
    ...
    with cols[0]:
        if st.button("Terms of Use"):
            safe_switch_page("app_pages/SeniorNav_terms.py")
    with cols[1]:
        if st.button("Privacy Policy"):
            safe_switch_page("app_pages/SeniorNav_privacy.py")
