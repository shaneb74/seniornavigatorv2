...
with left:
    st.markdown("## Planning for someone else?")
    st.write("Tell us who you’re supporting so we can tailor the experience for them.")

    name_default = (aud.get("recipient_name") or "").strip()
    entered_name = st.text_input(
        "Loved one’s preferred name (optional)",
        value=name_default,
        placeholder="e.g., Mom, Uncle James",
    )
    ...
    with col_continue:
        if st.button("Continue", type="primary", use_container_width=True):
            ...
            aud["entry"] = "proxy"
            care_context["person_name"] = trimmed or "Your Loved One"
...
with right:
    _render_hero(
        ...
        alt="Caregiver supporting older adult — planning for a loved one",
    )
