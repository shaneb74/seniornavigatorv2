...
"""Contextual Welcome - centered card, left text / right image, no pills, solid gating."""
...
def render(which: str = "you") -> None:
    ...
    # Optional theme injection if your app has it
    try:
    ...
    # Page-scoped CSS + centering wrapper
    st.markdown(
        """<style>
        ...
        .cw-card{
            ...
            box-shadow:0 30px 80px rgba(2,12,27,.12);
        }
        /* Headline + inputs */
        ...
        .name-row .stButton>button{ height:44px; border-radius:10px; font-weight:700; min-width:160px; }

        /* Remove the ghost bar look from TextInput wrapper */
        .stTextInput > div > div{ background:transparent !important; box-shadow:none !important; }
        .stTextInput input{ height:44px !important; border-radius:10px !important; }
    ...
    # Start centered viewport + card
    st.markdown('<div class="cw-viewport"><div class="cw-card">', unsafe_allow_html=True)
    ...
    with left:
        ...
        # Relationship for proxy flow (progressive after name)
        if entry == "proxy" and (aud.get("recipient_name") or "").strip():
            ...
            rel_label = st.selectbox(
                f"What's your relationship to {aud['recipient_name'] or 'them'}?", labels, index=idx, key="cw_relationship"
            )
            ...
            if aud["relationship_code"] == "other":
                aud["relationship_other"] = st.text_input(
                    "Briefly describe the relationship", value=aud.get("relationship_other") or "",
                    key="cw_relationship_other", label_visibility="collapsed",
                )
            else:
