...
def _ensure_care_context(entry: str, people: dict[str, object]) -> dict[str, object]:
    context = st.session_state.setdefault(
        "care_context",
        {"person_name": "Your Loved One", "gcp_answers": {}, "gcp_recommendation": None},
    )
    if entry == "self":
        context["person_name"] = "You"
    else:
        # Prefer the captured recipient name, but fall back to a friendly default.
        person_name = str(people.get("recipient_name") or context.get("person_name") or "Your Loved One")
        context["person_name"] = person_name
    return context


def _prime_audiencing_state(entry_override: str | None) -> dict[str, object]:
    state = ensure_audiencing_state()
    if entry_override:
        state["entry"] = entry_override
    elif state.get("entry") is None:
        state["entry"] = "proxy"
    ...
    _ensure_care_context(entry, people)
...
def render_contextual_welcome(entry_override: str | None = None) -> None:
    inject_theme()
    ...
    state = _prime_audiencing_state(entry_override)
    entry = state.get("entry") or "proxy"
